  ...
  /* Custom filtering function which will search data in column four between two values */
    ...
    $(document).ready(function() {
        ...
        $('#txt_filter').on( 'keyup', function () {
            ...
            save_filter(2005, $('#txt_filter').val(), $('#cbo_filter_status').val());
        });
...
<script>
    ...
    $(function(){
        ...
        $("#btnOK").click(function() 
        {
            ...
            jQuery.ajax({
                type: "POST",
                url: "<?php echo base_url(); ?>" + "configuration/revenue/check",
                dataType: 'json',
                data: {
                    revenue_id: $('#h_revenue_id').val(),
                    from: $('#txt_from').val(),
                    to: $('#txt_to').val()},
                success: function(res) {
                    if (res)
                    {
                        ...
                        {
                            $("#lbl_message").text("The Revenue is exists. Please try another Revenue.");
                            $("#lbl_message").show();
                            return;   
                        }
                        else
        ...
        $("#btnDelete").click(function() 
        {
            ...
            $('#myform').attr('action', '<?php echo base_url(); ?>configuration/revenue/delete');
            document.getElementById("myform").submit(); 
        });
    ...
    function edit(pID)
    {
        ...
        jQuery.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>" + "configuration/revenue/get",
            dataType: 'json',
            data: {
                revenue_id: pID},
            success: function(res) {
    ...
    <div class="row">
        <div class="col-lg-12">
            <table id="tbl" data-toolbar="#filter-bar" class="table table-striped table-bordered table-hover dt-responsive">
                ...
                <tbody>
                    <?php for ($i = 0; $i < count($revenuelist); ++$i) { ?>
                    <tr>
                        <td><?php echo ($i+1); ?></td>
                        <td class="all"><?php echo number_format($revenuelist[$i]->from, 2)." - ".number_format($revenuelist[$i]->to, 2) ; ?></td>
                        <td><?php if ($revenuelist[$i]->status == 1) {echo '<span class="text-active">Active</span>';} else {echo '<span class="text-inactive">In-active</span>';} ?></td>
                        <td><?php echo $revenuelist[$i]->created_user; ?></td>
                        <td><?php echo $revenuelist[$i]->created_date; ?></td>
                        <td class="hidden"><?php echo $revenuelist[$i]->status; ?></td>
                        <td class="all">
                            <button class="btn btn-success btn-xs" onclick="edit(<?php echo $revenuelist[$i]->revenue_id; ?>);">Edit</button>
                            <button class="btn btn-danger btn-xs" onclick="del(<?php echo $revenuelist[$i]->revenue_id; ?>);">Delete</button>
                        </td>
                    </tr>
                    <?php } ?>
                </tbody>
...
<form action="<?php echo base_url(); ?>configuration/revenue/add" id="myform" method="post" accept-charset="utf-8">
<div id="popupForm" class="modal-box">
    ...
    <div class="modal-body" style="max-height:300px;overflow-y: auto">
        <div class="row">
            <p id="lbl_message" class="alert alert-danger">The Revenue field is required.</p>
...
<input type="hidden" id="h_revenue_id" name="h_revenue_id" value="0">
