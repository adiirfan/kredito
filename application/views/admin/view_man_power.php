  ...
  /* Custom filtering function which will search data in column four between two values */
    ...
    $(document).ready(function() {
        ...
        $('#txt_filter').on( 'keyup', function () {
            ...
            save_filter(2004, $('#txt_filter').val(), $('#cbo_filter_status').val());
        });
...
<script>
    ...
    $(function(){
        ...
        $("#btnOK").click(function() 
        {
            ...
            jQuery.ajax({
                type: "POST",
                url: "<?php echo base_url(); ?>" + "configuration/man_power/check",
                dataType: 'json',
                data: {
                    man_power_id: $('#h_man_power_id').val(),
                    from: $('#txt_from').val(),
                    to: $('#txt_to').val()},
                success: function(res) {
                    if (res)
                    {
                        ...
                        {
                            $("#lbl_message").text("The Man Power is exists. Please try another Man Power.");
                            $("#lbl_message").show();
                            return;   
                        }
                        else
        ...
        $("#btnDelete").click(function() 
        {
            ...
            $('#myform').attr('action', '<?php echo base_url(); ?>configuration/man_power/delete');
            document.getElementById("myform").submit(); 
        });
    ...
    function edit(pID)
    {
        ...
        jQuery.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>" + "configuration/man_power/get",
            dataType: 'json',
            data: {
                man_power_id: pID},
            success: function(res) {
...
<div id="page-wrapper">
    ...
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header">Man Power
    ...
    <div class="row">
        <div class="col-lg-12">
            <table id="tbl" data-toolbar="#filter-bar" class="table table-striped table-bordered table-hover dt-responsive">
                ...
                <tbody>
                    <?php for ($i = 0; $i < count($man_powerlist); ++$i) { ?>
                    <tr>
                        <td><?php echo ($i+1); ?></td>
                        <td class="all"><?php echo $man_powerlist[$i]->from." - ".$man_powerlist[$i]->to ; ?></td>
                        <td><?php if ($man_powerlist[$i]->status == 1) {echo '<span class="text-active">Active</span>';} else {echo '<span class="text-inactive">In-active</span>';} ?></td>
                        <td><?php echo $man_powerlist[$i]->created_user; ?></td>
                        <td><?php echo $man_powerlist[$i]->created_date; ?></td>
                        <td class="hidden"><?php echo $man_powerlist[$i]->status; ?></td>
                        <td class="all">
                            <button class="btn btn-success btn-xs" onclick="edit(<?php echo $man_powerlist[$i]->man_power_id; ?>);">Edit</button>
                            <button class="btn btn-danger btn-xs" onclick="del(<?php echo $man_powerlist[$i]->man_power_id; ?>);">Delete</button>
                        </td>
                    </tr>
                    <?php } ?>
                </tbody>
...
<form action="<?php echo base_url(); ?>configuration/man_power/add" id="myform" method="post" accept-charset="utf-8">
<div id="popupForm" class="modal-box">
    ...
    <div class="modal-body" style="max-height:300px;overflow-y: auto">
        <div class="row">
            <p id="lbl_message" class="alert alert-danger">The Man Power field is required.</p>
        ...
        <div class="row">
            ...
            <div class="col-sm-8" style="display:inline-block">
                <input type="text" id="txt_from" name="txt_from" style="width:80px; display:inline" onkeydown="keydown(event);" maxlength="20" class="form-control" /> - 
                <input type="text" id="txt_to" name="txt_to" style="width:80px; display:inline" onkeydown="keydown(event);" maxlength="20" class="form-control" />
            </div>    
        </div>   
        <div class="row">
            <div class="col-sm-4">
...
<input type="hidden" id="h_man_power_id" name="h_man_power_id" value="0">
