...
<script type="text/javascript">
  ...
  /* Custom filtering function which will search data in column four between two values */
    ...
    function get_filter(pArray)
    {
        ...
        table.columns(8).search(pArray[1]).draw();
    }
</script>
...
<script>
    ...
    $(function(){
        ...
        $("#btnDelete").click(function() 
        {
            ...
            $('#myform').attr('action', '<?php echo base_url(); ?>workspace/user/delete/investor');
            document.getElementById("myform").submit(); 
        });
        ...
        jQuery.ajax({
            ...
            success: function(res) {
                if (res)
                {
                    ...
                    if (res.investor_type == "I")
                    {
                        $("#lbl_investor_type").text("Individual");
                        $("#lbl_company_name").text("");
                        $("#lbl_company_registration").text("");
                        $("#div_company_name").addClass("hidden");
                        $("#div_company_registration").addClass("hidden");
                    }
                    else
                    {
                        $("#lbl_investor_type").text("Corporate");
                        ...
                        $("#div_company_registration").removeClass("hidden");
                    }
        ...
        <div class="col-lg-12">
            <table id="tbl" data-toolbar="#filter-bar" class="table table-striped table-bordered table-hover dt-responsive">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><?php echo ($i+1); ?></td>
                        <td><?php echo $investor_list[$i]->salutation_name; ?></td>
                        <td class="all"><?php echo $investor_list[$i]->full_name; ?></td>
                        <td><?php echo $investor_list[$i]->mobile_phone; ?><br /><?php echo $investor_list[$i]->email; ?></td>
                        <td><?php if ($investor_list[$i]->investor_type == "I") {echo 'Individual';} else {echo 'Corporate ('.$investor_list[$i]->company_name.')';} ?></td>
                        <td class="all"><a target="_blank" href="<?php echo base_url(); ?>workspace/investment/list/<?php echo $investor_list[$i]->user_id; ?>"><?php echo number_format($investor_list[$i]->total_fund, 2); ?></a></td>
                        <td><?php if ($investor_list[$i]->status == 1) {echo '<span class="text-active">Active</span>';} else {echo '<span class="text-inactive">In-active</span>';} ?></td>
                        <td><?php echo $investor_list[$i]->created_date; ?></td>
                        <td class="hidden"><?php echo $investor_list[$i]->status; ?></td>
                        <td class="all">
                            <button class="btn btn-success btn-xs" onclick="edit(<?php echo $investor_list[$i]->user_id; ?>);">Edit</button>
                            <button class="btn btn-danger btn-xs" onclick="del(<?php echo $investor_list[$i]->user_id; ?>);">Delete</button>
                        </td>
                    </tr>
                    <?php } ?>
                </tbody>
...
<form action="<?php echo base_url(); ?>workspace/user/update/investor" id="myform" method="post" accept-charset="utf-8">
<div id="popupForm" class="modal-box">
    ...
    <div class="modal-body" style="max-height:300px;overflow-y: auto">
        ...
        <div class="row">
            ...
            <div class="col-sm-8">
                <label id="lbl_investor_type" class="normal"></label>
            </div>    
        </div>  
        <div class="row" id="div_company_name">
            <div class="col-sm-4">
                Company Name:
            </div>            
            <div class="col-sm-8">
        ...
        <div class="row" id="div_company_registration">
            <div class="col-sm-4">
                Registration No.:
            </div>            
            <div class="col-sm-8">
...
<div id="popupList" class="modal-box">
    <header>
      <h4><span id="lbl_add_new">Add new record</span>
        <a href="#" class="js-modal-close2 close" id="btn_close">x</a>
      </h4>
