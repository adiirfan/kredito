  ...
  /* Custom filtering function which will search data in column four between two values */
    ...
    $(document).ready(function() {
        ...
        $('#txt_filter').on( 'keyup', function () {
            ...
            save_filter(3002, $('#txt_filter').val());
        });
...
<script>
    ...
    function edit(pID)
    {
        jQuery.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>" + "workspace/loan/get",
            dataType: 'json',
            data: {
                loan_id: pID},
            success: function(res) {
                if (res)
                {
                    ...
                    $("#lbl_b_full_name").text(res.b_full_name);
...
<div id="page-wrapper">
    ...
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header">Loan - Processing
            </h1>
        </div>
    </div>
    <div class="row">
        <div class="col-lg-12">
            <table id="tbl" data-toolbar="#filter-bar" class="table table-striped table-bordered table-hover dt-responsive">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><?php echo ($i+1); ?></td>
                        <td class="all"><?php echo $loan_list[$i]->code; ?></td>
                        <td><?php echo $loan_list[$i]->borrower_name; ?></td>
                        <td><?php echo $loan_list[$i]->b_mobile_phone; ?><br /><?php echo $loan_list[$i]->b_email; ?></td>
                        <td>MYR <?php echo number_format($loan_list[$i]->amount, 2); ?><br /><?php echo $loan_list[$i]->period; ?> mths</td>
                        <td><?php echo $loan_list[$i]->submitted_date; ?></td>
                        <td><?php echo $loan_list[$i]->investment_code; ?></td>
                        <td class="all">
                            <a href="<?php echo base_url(); ?>workspace/loan/detail/<?php echo $loan_list[$i]->code; ?>" class="btn btn-success btn-xs" target="_blank">View Detail</a>
                            <button class="btn btn-info btn-xs" onclick="edit(<?php echo $loan_list[$i]->loan_id; ?>);">Update Status</button>
                            <a href="<?php echo base_url(); ?>workspace/loan/matching/<?php echo $loan_list[$i]->code; ?>" class="btn btn-primary btn-xs" target="_blank">Matching</a>
                        </td>
                    </tr>
                    <?php } ?>
                </tbody>
...
<form action="<?php echo base_url(); ?>workspace/loan/status" id="myform" method="post" accept-charset="utf-8">
...
<input type="hidden" id="h_loan_id" name="h_loan_id" value="0">
...
<input type="hidden" id="h_menu_id" name="h_menu_id" value="3002">
<input type="hidden" id="h_menu_name" name="h_menu_name" value="processing">
</form>