...
;
    		...
    		case "next":
    		{
	            ...
	            else
	            {
	            	...
	            	if (get_cookie("user_id") == "")
	    			{
						...
						$loan_id = $data["folder_code"];
						//dihilangkan edu
		                //$this->model_user->update_other($param);
						//dihilangkan edu
						$config = array();
						$config['upload_path'] = './upload/loan/'. $loan_id.'/KTP/';
						...
						$this->load->library('upload', $config, 'ktpupload'); // Create custom object for cover upload
						$this->ktpupload->initialize($config);
						$this->ktpupload->overwrite = true;
						$upload_ktp = $this->ktpupload->do_upload('KTP');
						$cover_data1 = $this->ktpupload->data();
						$cd1= $cover_data1['file_name'];
						
						$configs = array();
						$configs['upload_path'] = './upload/loan/'. $loan_id.'/SIUP/';
						$configs['allowed_types'] = 'gif|jpg|png|';
						$configs['max_size'] = '10000000';
						$configs['max_width'] = '99999';
						$configs['max_height'] = '9999';
						$configs['file_name'] = 'siup';
						

						$this->load->library('upload', $configs, 'ktpupload'); // Create custom object for cover upload
						$this->ktpupload->initialize($configs);
						$this->ktpupload->overwrite = true;
						$upload_siup = $this->ktpupload->do_upload('SIUP');
						$cover_data2 = $this->ktpupload->data();
						$cd2= $cover_data2['file_name'];
						$configa = array();
						$configa['upload_path'] = './upload/loan/'. $loan_id.'/NPWP/';
						$configa['allowed_types'] = 'gif|jpg|png|';
						$configa['max_size'] = '1000000';
						$configa['max_width'] = '99999';
						$configa['max_height'] = '9999';
						$configa['file_name'] = 'npwp';

						$this->load->library('upload', $configa, 'ktpupload'); // Create custom object for cover upload
						$this->ktpupload->initialize($configa);
						$this->ktpupload->overwrite = true;
						$upload_npwp = $this->ktpupload->do_upload('NPWP');
						
						$cover_data = $this->ktpupload->data();
						$cd3= $cover_data['file_name'];
						
						$file_name_array=array($cd1,$cd2,$cd3);
						$document_type_array=array("KTP","SIUP","NPWP");
			...
			case "info":
    		{
					...
					$tenor_rumah=$this->input->post('h_waktu_rumah');
						...
						if($this->input->post('h_selected_tujuan')=='1'){
							...
							if($this->input->post('h_selected_month',true) != "tes"){
	...
	function update_loan($user_id)
	{
		...
		$this->set_cookies->setBMonnth($this->input->post("period"));
