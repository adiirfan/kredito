...
class Send_mail {
    ...
    public function new_admin_to_user($sys_user_id) 
    {
		...
		$lMessage = 
            "Hai ".$row->full_name.",".
            "<BR><BR>".
			"Terima kasih telah mendaftar mendaftar di Kredito.co.id.".
			"Akun Anda hampir siap ! Anda hanya selangkah lagi mengakses akun Kredito Anda. ".
            "<BR><BR>".
            ...
            "<BR>".
            "Jika link tersebut tidak bekerja, maka anda dapat menyalin link ini ke browser web Anda<BR>".
            "<a target='_blank' href='".base_url()."admin/activation/".$row->dynamic_code."'>".base_url()."admin/activation/".$row->dynamic_code."</a>.<BR>";
        
        $subject = 'Selamat datang Kredito Administrator - akun anda telah diaktifkan';
    ...
    public function new_registration_to_user($user_id) 
    {
        ...
        $subject = 'Selamat datang di Kredito.co.id - Akun anda sudah dibuat';
        $message = $lEmailHeader.$lMessage.$lEmailFooter; 
        $this->send($row->email, $subject, $message);
    }
	public function new_loan_to_user($loan_id) 
    {
        ...
        $subject = 'Selama datang di Kredito.co.id - akun anda telah dibuat';
        $message = $lEmailHeader.$lMessage.$lEmailFooter;
	...
	public function multiple_loan_to_user($user_id,$code_loan,$email,$new=null) 
    {
       ...
       // $product = $ci->model_loan->get_loan_product_by_code($code_loan);
        ...
        $subject = 'Selama datang di Kredito.co.id - pengajuan anda berhasil telah kami terima';
        $message = $lEmailHeader.$lMessage.$lEmailFooter;
        $this->send($row_user->email, $subject, $message);
		}else if($new == 1){
			$lMessage = 
	 ...
	 public function new_loan_to_olduser($loan_id) 
    {
		...
		$lMessage = 
                ...
                "<BR><BR>".
                "<a target='_blank' href='".base_url()."'>Log in  Kredito.com untuk melihat status peminjaman anda</a>.<BR>".
                "<BR>".
                "Jika link tersebut tidak bekerja, maka anda dapat menyalin link ini ke browser web Anda<BR>".
        ...
        $subject = 'Kredito.co.id - Pengajuan pinjaman anda telah kami terima';
        $message = $lEmailHeader.$lMessage.$lEmailFooter;
        $this->send($row_user->email, $subject, $message);
    }
    ...
    public function request_password_to_web_user($email) 
    {
        ...
        else
        {
            ...
            $subject = 'Kredito.co.id: Permintaan ubah password';
            $message = $lEmailHeader.$lMessage.$lEmailFooter;
            $this->send($row->email, $subject, $message); 
        }
	...
	public function status_pinjaman($status,$user_id,$kode) 
    {
		...
		if($status == 4){
			$lMessage = 
                ...
                "<BR><BR>".
                "Silahkan hubungi tim Kredito untuk informasi lebih lanjut".
                "<BR><BR>".
        ...
        $subject = 'Kredito.co.id - Pinjaman anda telah di verifikasi';
		} else if($status == 7){
			$lMessage = 
                "Hi ".$row_user->full_name.",".
                "<BR><BR>".
        ...
        $subject = 'Kredito.co.id - Pinjaman anda ditolak';
		}
		else if($status == 8){
			$lMessage = 
                "Hi ".$row_user->full_name.",".
	...
	public function status_pinjaman_refinance($status,$kode,$refinance) 
    {
       ...
       // $row_user = $ci->model_user->get($user_id);
        ...
        $subject = 'Kredito.co.id - Pinjaman anda sedang dalam proses';
		} else if($status == 4){
			$lMessage = 
                "Hi ".$row_user->loan_name.",".
                "<BR><BR>".
        ...
        $subject = 'Kredito.co.id - Pinjaman anda berhasil';
		}else if($status == 5){
			$lMessage = 
                "Hi ".$row_user->loan_name.",".
                "<BR><BR>".
		...
		if($status == '1'){
			$lMessage = 
              //  "Kepada ".$row_user->full_name.",".
                ...
                "<BR>".
                "Untuk informasi lebih lanjut silahkan hubungi Costumer Service Kredito<BR>";
                  
  
			$subject = 'Kredito.co.id - Status Pengajuan Pinjaman Anda';
		}else if($status == '2'){
	...
	public function investor_active($user_id) 
    {
		...
		$lMessage = 
                "Hi ".$row_user->full_name.",".
                "<BR><BR>".
				"Status anda sebagai investor di Kredito.co.id telah diaktifkan".
                "<BR><BR>".
                "Sekarang anda dapat melakukan bid kepada calon peminjam sesuai dengan limit dana yang diterima oleh Kredito.com".
                "<BR><BR>".
        ...
        $subject = 'Kredito.co.id - Status anda sebagai investor telah diaktifkan';
        $message = $lEmailHeader.$lMessage.$lEmailFooter;
        $this->send($row_user->email, $subject, $message);
    }
	...
	$ci->load->model('model_backend');
		...
		$lMessage = 
                ...
                "<BR><BR>".
                "Total investasi anda saat ini di Kredito sebesar Rp. <b>".$ci->model_backend->rupiah($totaldeposit) ."</b>".
        ...
        $subject = 'Kredito.co.id - Dana investasi anda telah kami terima';
        $message = $lEmailHeader.$lMessage.$lEmailFooter;
        $this->send($row_user->email, $subject, $message);
    }
		 ...
		 public function pengajuan_penambahan_saldo($investment_code) 
    {
        ...
        $subject = 'Kredito - Investasi anda berhasil di submit';
        $message = $lEmailHeader.$lMessage.$lEmailFooter;
        $this->send($row_user->email, $subject, $message);
    }
...
//Administrator
 public function new_investor_application_to_admin($user_id) 
        ...
        else
        {
            $lMessage = 
               ...
               "Investasi anda berhasil di submit . Nomor referensi pengajuan deposit anda adalah : <b>".$row->user_code."</b>".
                ...
                "Kami akan memverifikasi investasi anda paling lambat 24 jam.".
                "<BR><BR>".
                "<a target='_blank' href='".base_url()."'>Silahkan Login  Kredito untuk melihat status deposit anda</a>.<BR>".
                "<BR>".
	...
	public function activation_bid($user_id,$loan_id,$code,$amount) 
    {
        ...
        $subject = 'Kredito.co.id - Konfirmasi BID anda';
        $message = $lEmailHeader.$lMessage.$lEmailFooter;
        $this->send($row_user->email, $subject, $message);
    }
