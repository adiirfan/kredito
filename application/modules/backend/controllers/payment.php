...
class payment extends CI_Controller {
	...
	$this->template->display('view_payment_list',$data);		
	}
	
	public function show_payment($id=NULL)
	{ 
	
	$data=$this->model_backend->form_payment($id);
...
$this->send_mail->status_pinjaman_refinance($this->input->post('loan_status'),$this->input->post('user_id'),$this->input->post('loan_code'),$this->input->post('product_name'));
	...
	public function update_payment($id=NULL)
	{
		...
		$data = array(
				'schedule_2' => $this->input->post('schedule_2'),
				'instalment_2' => $this->input->post('instalment_2'),
		);
		$where ='loan_repayment_id ='.$this->input->post('loan_repayment_id');
		$this->db->update('loan_repayment', $data, $where );
		
		
		echo json_encode(array("status" => TRUE,"status2" => $this->input->post('loan_repayment_id')));
	}
	...
	public function detail($pParam2)
	{
		$data['segment'] = $this->uri->segment(2);
				...
				$data['repayment']=$this->model_loan_repayment->get_list_by_loan($loan->loan_id);
				
				

                $this->template->display('view_detail_payment',$data);
